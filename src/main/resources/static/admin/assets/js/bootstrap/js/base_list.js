...
(function($) {
	$.extend({
		table:{
            ...
            oTableInitworkorder:function(parms){//初始化表单
                $.table._option=parms;
                var oTableInit = new Object();
                //初始化Table
                oTableInit.Init = function () {
                    $('#dataTable').bootstrapTable({
                        ...
                        responseHandler:function(res){
                            res.rows.forEach(function(item){
                                if(item.workingState===0){
                                    item.workingState='未处理';
                                }
                                if(item.workingState===1){
                                    item.workingState='完成';
                                }
                                if(item.workingState===4){
                                    item.workingState='取消';
                                }
                                if(item.orderCreateTime){
                                item.orderCreateTime = $.table.transferTime(item.orderCreateTime);}
                                if(item.orderFinishTime){
                                    item.orderFinishTime = $.table.transferTime(item.orderFinishTime);}
                        ...
                        onLoadSuccess: function(){  //加载成功时执行
                            ...
                            // res.rows.0.orderCreateTime = $.table.transferTime(res.rows.0.orderCreateTime);
            ...
            oTableInitmaillog:function(parms){//初始化表单
                $.table._option=parms;
                var oTableInit = new Object();
                //初始化Table
                oTableInit.Init = function () {
                    $('#dataTable').bootstrapTable({
                        ...
                        responseHandler:function(res){
                            res.rows.forEach(function(item){
                                if(item.pushTime){
                                    item.pushTime = $.table.transferTime(item.pushTime);}
                            });
            ...
            oTableInitworkorderdo:function(parms){//初始化表单
                $.table._option=parms;
                var oTableInit = new Object();
                //初始化Table
                oTableInit.Init = function () {
            ...
            oTableInitworkorderundo:function(parms){//初始化表单
                $.table._option=parms;
                var oTableInit = new Object();
                //初始化Table
                oTableInit.Init = function () {
                    $('#dataTable').bootstrapTable({
                        contentType: "application/x-www-form-urlencoded",
                        url:parms.dataUrlundo,     //请求后台的URL（*）
                        method: 'post',           //请求方式（*）
                        toolbar: '#exampleTableEventsToolbar',        //工具按钮用哪个容器
                        striped: true,           //是否显示行间隔色
                        cache: false,            //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		...
		operate:{
            ...
            setfireon: function(id,options) {
                var url = $.table._option.setfireonUrl.replace("{id}", id);
                console.log(options)
                $.modal.postfirestate(url)
            },

            setfireoff: function(id,options) {
                var url = $.table._option.setfireoffUrl.replace("{id}", id);
                console.log(options)
                $.modal.postfirestate(url)
            },
